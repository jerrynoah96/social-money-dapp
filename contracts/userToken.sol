...
contract UserToken is Context, Ownable {
 // using SafeMath for uint256;
  ...
  uint256 public _maxAllowedBalance;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
     ...
     if(excludedAddress[recipient] == false){
         require((amount+_balances[recipient]) <= _maxAllowedBalance,'We dont want this person to be whale');
     }
    ...
    if(_balances[sender] == 700000*(10**_decimals)){
         _autoMint();
     }

  }
  
  function _autoMint() internal{
      uint256 addedAmount = 700000*(10**_decimals);
     _balances[msg.sender]+= addedAmount;
     emit AutoMint(addedAmount);
  }
